...
void FSceneTexturesConfig::Init(const FSceneTexturesConfigInitSettings& InitSettings)
{
	...
	// Note that the GetDefaultMSAACount call is mirrored in the FSceneRenderer constructor where custom render passes are initialized.
	// Custom render passes don't support MSAA, and use a different config with MSAA disabled, when its enabled for the main view.  If
	// this calculation of NumSamples changes, that corresponding code would need to be updated.
